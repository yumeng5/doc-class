...
def read_file(data_dir, with_evaluation):
    data = []
    target = []
    with open(join(data_dir, 'cleaned.txt'), 'rt', encoding='utf-8') as f:
        for row in f:
            data.append(row.strip())
    if with_evaluation:
        with open(join(data_dir, 'truth.txt'), 'rt', encoding='utf-8') as f:
            for row in f:
                target.append(int(row.strip()))
        y = np.asarray(target)
...
def load_cnn(dataset_name, sup_source, num_keywords=10, with_evaluation=True, truncate_len=None):
    ...
    if sup_source == 'names' or sup_source == 'keywords':
        keywords = load_keywords(data_path, sup_source)
        return x, y, word_counts, vocabulary, vocabulary_inv, len_avg, len_std, keywords, perm, truncate_len
        ...
        return x, y, word_counts, vocabulary, vocabulary_inv, len_avg, len_std, keywords, sup_idx, perm, truncate_len
...
def load_rnn(dataset_name, sup_source, num_keywords=10, with_evaluation=True, truncate_len=None):
    ...
    if sup_source == 'names' or sup_source == 'keywords':
        keywords = load_keywords(data_path, sup_source)
        return x, y, word_counts, vocabulary, vocabulary_inv, len_avg, len_std, keywords, perm, [truncate_doc_len, truncate_sent_len]
    elif sup_source == 'docs':
        if dataset_name == 'nyt':
            class_type = 'topic'
        elif dataset_name == 'agnews':
        ...
        return x, y, word_counts, vocabulary, vocabulary_inv, len_avg, len_std, keywords, sup_idx, perm, [truncate_doc_len, truncate_sent_len]
