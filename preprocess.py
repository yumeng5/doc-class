...
def csv2txt(data_dir):
    ...
    with open(join(data_dir, 'text.txt'), 'wt', encoding='utf-8') as outfile:
        for row in data:
            outfile.write(row + '\n')
    with open(join(data_dir, 'truth.txt'), 'wt', encoding='utf-8') as outfile:
        for row in labels:
...
if __name__ == "__main__":
    ...
    clean_file(data, dataset, out_file=args.out_file)
